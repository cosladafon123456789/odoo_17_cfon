...
class SaleOrder(models.Model):
    ...
    is_rebu = fields.Boolean(string='Es REBU', default=True)
    ...
    def create(self, vals):
        ...
        # Sync to lines if order is created with is_rebu = True
        if vals.get('is_rebu') and order.order_line:
            order.order_line.write({'is_rebu': True})
        return order
    ...
    def write(self, vals):
        ...
        for order in self:
            if 'is_rebu' in vals:
                # Only update lines if they differ
...
class SaleOrderLine(models.Model):
    ...
    # sinvoice_status = fields.Selection(related="order_id.invoice_status",string='Invoice Status')
...
class StockMove(models.Model):
    ...
    def _action_assign(self):
        ...
        for move in self.filtered(lambda x:x.imei_ids):
            # skip_moves |= move
            move.move_line_ids.unlink()
