...
class HelpdeskTicket(models.Model):
    ...
    # Texto de estado según días (según petición del usuario)
    ...
    @api.depends('linked_order_id.date_order', 'create_date')
    def _compute_days_since_purchase(self):
        for t in self:
            days = 0
            order = getattr(t, 'linked_order_id', False)
    ...
    def _compute_purchase_status_text(self):
        for t in self:
            if t.days_since_purchase and t.days_since_purchase > 730:
                t.purchase_status_text = "GARANTIA EXPIRADA"
            elif t.days_since_purchase and t.days_since_purchase > 21:
                # Nota: se usa la ortografía exacta solicitada por el usuario
