...
# --- PATCH FOR SIGN OUT ---
    ...
    checkout_calendar = request.env['resource.calendar.attendance'].search([
        ('dayofweek', '=', str(day_week)),
        ('day_period', '=', 'afternoon'),
        ('calendar_id', '=', calendar_id.id)
    ], limit=1)

    if checkout_calendar:
        shiftHour = math.floor(checkout_calendar.hour_to)
        shiftMinute = (checkout_calendar.hour_to - shiftHour) * 60
