...
class StockPicking(models.Model):
    ...
    def _cf_send_whatsapp_tecomprotumovil(self, partner):
        ...
        template = Template.search([("name", "=", "tecomprotumovil"), ("state", "=", "approved")], limit=1)
        if not template:
            # Buscar por technical name también
            template = Template.search([("technical_name", "=", "tecomprotumovil"), ("state", "=", "approved")], limit=1)
        if not template:
