...
class CfDevolucionDashboard(models.TransientModel):
    ...
    def action_open_graph_motivos(self):
        ...
        return {'type':'ir.actions.act_window','name':'Motivos de devolución','res_model':'sale.order','view_mode':'graph','domain':domain,'context':ctx}
    def action_open_graph_tipo_error(self):
        ...
        return {'type':'ir.actions.act_window','name':'Distribución por tipo de error','res_model':'sale.order','view_mode':'graph','domain':[],'context':ctx}
    def action_open_graph_mes(self):
        self.ensure_one()
        return {'type':'ir.actions.act_window','name':'Evolución mensual','res_model':'sale.order','view_mode':'graph','domain':[],'context':{'graph_measure':'__count','graph_mode':'line','group_by':['fecha_devolucion:month']}}
