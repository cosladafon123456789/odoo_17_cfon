...
class RepairActionWizard(models.TransientModel):
	...
	action_type = fields.Selection(
		[
			...
			('exclusive', 'Exclusion de Garantia'),
			('rma', 'RMA'),
		],
		string="Action Type")
	...
	def action_rma(self):
		...
		if not vendors:
			raise ValidationError(_("Please select vendors in line."))
		...
		picking_vals = {
			'sale_id': sale.id,
			'origin': sale.name,
			'partner_id': vendors and vendors[0].id,
		...
		# set status
		rma_id = self.env['repair.status'].search([('repair_status', '=', 'rma')])
		if not rma_id:
			raise ValidationError(('Please set RMA as status in setting'))
		sale.repair_status_ids = [(4, rma_id.id)]

		return True

	def action_exclusive_return(self):
		active_id = self.env.context.get('active_id')
		sale = self.env['sale.order'].browse(active_id)
		if not sale:
		...
		so_picking_id = stock_picking_model.search([('sale_id', '=', sale.id)], limit=1)
		...
		for line in self.line_ids:
			if line.product_id.id not in out_moves.mapped('product_id').ids:
				out_moves |= self.env['stock.move'].create({
					'name': line.product_id.name,
					'product_id': line.product_id.id,
					'product_uom_qty': line.quantity,
					'product_uom': line.sale_line_id.product_uom.id,
			...
			find_out_move = out_moves.filtered(lambda x: x.product_id == line.product_id)
			# print(find_out_move)
			move_in_line = self.env['stock.move.line'].create({
				'move_id': find_out_move.id,
				'product_id': line.product_id.id,
				'product_uom_id': line.product_id.uom_id.id,
				'picking_id': find_out_move.picking_id.id, 
				'quantity': line.quantity,
				'location_id': find_out_move.location_id.id,
				'location_dest_id': find_out_move.location_dest_id.id,
				'lot_name': line.lot_id.name,
				'lot_id': line.lot_id.id,
			})
			# print(move_in_line)
		...
		msg_body_in = _('Orden devuelta con Exclusion de Garantia, IN Picking : %s') % picking_in.name
		...
		msg_body_out = _('Orden devuelta con Exclusion de Garantia, OUT Picking : %s') % picking_out.name
		sale.message_post(body=msg_body_out)
		# sale.repair_status = 'return_to_customer'

		return_to_exclusive_id = self.env['repair.status'].search([('repair_status', '=', 'exclusive')])
		if not return_to_exclusive_id:
			raise ValidationError(('Please set exclusive  return as repair status in setting'))
		sale.repair_status_ids = [(4, return_to_exclusive_id.id)]
		return True
	...
	def action_return_to_customer(self):
		...
		return_to_customer_id = self.env['repair.status'].search([('repair_status', '=', 'return_to_customer')])
		if not return_to_customer_id:
			raise ValidationError(('Please set return to customer as status in setting'))
		sale.repair_status_ids = [(4, return_to_customer_id.id)]
	...
	def reverse_moves(self, is_modify=False):
		...
		if moves_to_redirect:
			...
			refund_id = self.env['repair.status'].search([('repair_status', '=', 'refund')])
			if not refund_id:
				raise ValidationError(('Please set refund as status in setting'))
			sale.repair_status_ids = [(4, refund_id.id)]
	...
	def action_replace(self):
		...
		replace_id = self.env['repair.status'].search([('repair_status', '=', 'replace')])
		if not replace_id:
			raise ValidationError(('Please set replace as status in setting'))
		sale.repair_status_ids = [(4, replace_id.id)]
...
class RepairActionLineWizard(models.TransientModel):
	...
	def get_vendor_ids(self):
		for rec in self:
			partner_ids = rec.product_id.mapped('seller_ids').mapped('partner_id')
			if not partner_ids:
				partner_ids = rec.lot_id.product_id.mapped('seller_ids').mapped('partner_id')
