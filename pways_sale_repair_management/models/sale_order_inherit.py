...
class RepairStatus(models.Model):
	...
	string='Estado De Reparacion',copy=False)
	message = fields.Char()
...
class SaleOrder(models.Model):
	...
	delivery_check = fields.Boolean("Delivered", compute='_compute_delivery_check')
	delivery_done = fields.Boolean('Received',copy=False)
	is_warranty = fields.Boolean('Warranty')
	repair_status_ids = fields.Many2many('repair.status',copy=False)
	repair_status = fields.Selection(
	[
		('received', 'Recibido'),
		('repair', 'Reparación'),
	...
	def _compute_rma_count(self):
		for rec in self:
			count_of_rma = self.env['stock.picking'].search_count([('sale_id', '=', rec.id),('is_rma', '=', True)])
			rec.rma_count = count_of_rma

	def button_view_rma_orders(self):
		activities = self.env['stock.picking'].search([('sale_id', '=', self.id),('is_rma', '=', True)])
		action = self.env["ir.actions.actions"]._for_xml_id("stock.action_picking_tree_all")
		action['domain'] = [('id', 'in', activities.ids)]
		return action

	def button_recieved(self):
		received_id = self.env['repair.status'].search([('repair_status', '=', 'received')], limit=1)
		if not received_id:
			raise ValidationError(('Please set received as status in sales setting'))
		self.repair_status_ids = [(4, received_id.id)]
	...
	def button_replace(self):
		...
		# self.repair_status_ids = [(4, replace_id.id)]
	...
	def button_refund(self):
		...
		# self.repair_status_ids = [(4, refund_id.id)]
	...
	def button_return_exclusive(self):
		self.ensure_one()
		...
		# self.repair_status_ids = [(4, return_to_customer_id.id)]
		
		repair_lines = self.order_line.filtered(lambda l: l.create_repair_order)
		if not repair_lines:
			raise ValidationError(_("No lines are selected to create return exclusive"))
		...
		for (product_id, lot_id, line_id), qty in product_lot_qty_map.items():
			...
			if quantity > 0:
				...
				created = True
		if not created:
			raise ValidationError(_("You don't have products to exclusive Return."))
	...
	def button_rma(self):
		...
		# self.repair_status_ids = [(4, rma_id.id)]
