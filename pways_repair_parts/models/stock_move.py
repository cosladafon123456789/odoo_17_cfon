...
class LotExpiryDates(models.Model):
	_name = 'lot.expiry.dates'
...
class SaleOrderLine(models.Model):
	...
	lot_date_ids = fields.Many2many('lot.expiry.dates', compute='get_lot_dates',store=True)
	lot_ids = fields.Many2many('stock.lot', compute='get_lot_dates', store=True)

	@api.depends('move_ids','move_ids.move_line_ids')
	def get_lot_dates(self):
		for rec in self:
			expiration_data_ids = self.env['lot.expiry.dates']
			expiration_lots = rec.move_ids.mapped('move_line_ids').mapped('lot_id')
			for lot in expiration_lots:
				expiration_data_ids |= self.env['lot.expiry.dates'].create({
					'name' : "%s" %(lot.expiration_date.date()) if lot.expiration_date else ''
				})
			rec.lot_date_ids = expiration_data_ids.ids
			rec.lot_ids = expiration_lots.ids

class PurchaseOrder(models.Model):
	_inherit = "purchase.order"

	repair_id = fields.Many2one('repair.order')
	is_part = fields.Boolean(copy=False)
